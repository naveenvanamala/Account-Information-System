...
{
    ofstream outfile;
    outfile.open("acc_data", ios::binary|ios::app);
    read();
...
void account_info::read_file()
{
    ...
    cout<<"\t\t*******************************************\n\n";
    ifstream infile;
    infile.open("acc_data", ios::binary);
    if(!infile)
    {
        cout<<"\nError in Opening!\n File Not Found!!"<<endl;
        return;
...
void account_info::del()
{
    ...
    cin>>n;
    fstream tmpfile;
    tmpfile.open("temp", ios::out|ios::binary);
    infile.seekg(0);
...
void account_info::edit()
{
    ...
    int n;
    fstream iofile;
    iofile.open("acc_data", ios::in|ios::binary);
    if(!iofile)
    {
        cout<<"\nError in opening! File Not Found!!"<<endl;
        return;
    ...
    if(count<=0)
    {
        cout<<"there are no records in the file ";
        return;
    }
    cout<<"\n There are "<<count<<" record in the file";
    ...
    show();
    iofile.close();
    iofile.open("acc_data", ios::out|ios::in|ios::binary);
    ...
    while(choice!=6)
    {
        ...
        switch(choice)
        {
            ...
            case 6:
                break;
            default:
                cout<<"\nEnter corret choice(i.e, 1-6 only)\n\n";
        }
    }
    return 0;
}
